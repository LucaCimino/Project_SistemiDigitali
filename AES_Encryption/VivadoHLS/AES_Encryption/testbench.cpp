#include "main.h"
#include <cstdio>

int main()
{
	/* 4096 Byte plaintext */
	uint8_t plaintext[] = {
		0x04, 0xb4, 0x56, 0xee, //
		,  // 256 Byte
										0x04, 0xb4, 0x56, 0xee, //
										,  // 512 Byte
																0x04, 0xb4, 0x56, 0xee, //
																		,  // 768 Byte
																								0x04, 0xb4, 0x56, 0xee, //
																										,  // 1024 Byte
																																0x04, 0xb4, 0x56, 0xee, //
																																		,  // 12.. Byte
																																								0x04, 0xb4, 0x56, 0xee, //
																																										,  // 15.. Byte
																																																0x04, 0xb4, 0x56, 0xee, //
																																																		,  // 17.. Byte
																																																								0x04, 0xb4, 0x56, 0xee, //
																																																										, //
																																																																		,  // 256 Byte
																																																																										0x04, 0xb4, 0x56, 0xee, //
																																																																										,  // 512 Byte
																																																																																0x04, 0xb4, 0x56, 0xee, //
																																																																																		,  // 768 Byte
																																																																																								0x04, 0xb4, 0x56, 0xee, //
																																																																																										,  // 1024 Byte
																																																																																																0x04, 0xb4, 0x56, 0xee, //
																																																																																																		,  // 12.. Byte
																																																																																																								0x04, 0xb4, 0x56, 0xee, //
																																																																																																										,  // 15.. Byte
																																																																																																																0x04, 0xb4, 0x56, 0xee, //
																																																																																																																		,  // 17.. Byte
																																																																																																																								0x04, 0xb4, 0x56, 0xee, //
																																																																																																																										};	// 2048 Bytes




	/* 4096 Bytes chiper */
	hls::stream<stream_type> cipher;

	int parallelismo = 256;

	/* Input:
	 *   - plaintext -> 4096 Bytes di testo da cifrare
	 *
	 * Output:
	 *   - cipher    -> 4096 Bytes di testo cifrato
	 *
	 */

	int i, block;
	hls::stream<stream_type> i_plaintext;
	stream_type tmp;

	for(i = 0; i < BLOCK_SIZE*parallelismo; i++) {
		tmp.user = 1;
		tmp.data = plaintext[i];

		if(i == (BLOCK_SIZE*parallelismo)-1)
			tmp.last = 1;
		else
			tmp.last = 0;

		i_plaintext.write(tmp);
	}


	single_block_AES_encrypt(i_plaintext, cipher);


	for(block = 0; block < parallelismo; block++)
	{
		printf(" ------ BLOCK %d -------\n", block);
		printf("Plaintext:\t");
		for (i = 0; i < 4; i++) {
			printf("%02x %02x %02x %02x ", plaintext[block*16 + 4*i+0], plaintext[block*16 + 4*i+1],
										   plaintext[block*16 + 4*i+2], plaintext[block*16 + 4*i+3]);
		}
		printf("\n");

		uint8_t output[BLOCK_SIZE];

		printf("TLAST : \t");
		for(i = 0; i < BLOCK_SIZE; i++) {
			tmp = cipher.read();
			output[i] = tmp.data;
			printf("%d ", (int)tmp.last);
		}
		printf("\n");

		printf("Cipher:\t\t");
		for (i = 0; i < 4; i++) {
			printf("%02x %02x %02x %02x ", output[4*i+0], output[4*i+1], output[4*i+2], output[4*i+3]);
		}
		printf("\n\n");
	}

}



